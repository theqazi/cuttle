...
| Field          | Value                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status         | Draft v1.3 (v0 at `docs/archive/PRD-v0-2026-04-25.md`; v1 at git `23455db`; v1.1 at git `b1913eb`; v1.2 at git `5c0a741`). v1.3 incorporates findings from `docs/adversarial-review-prd-v1.2.md` (Wolverine + Black Panther, Step 8 of threat-model skill). Pending pruning to v3 + seal, then TDD. Karpathy review (handoff path #2) is parallel-stream; can run before or after seal. |
| Owner          | Mohammed Qazi (sirajuddin.qazi@gmail.com)                                                                                                                                                                                                                                                                                                                                               |
| Created        | 2026-04-25 (v0); revised 2026-04-26 (v1, v1.1, v1.2, v1.3)                                                                                                                                                                                                                                                                                                                              |
...
- **Credential vault** (per D-2026-04-26-08, T-002, CC-2):
  ...
  - **Keychain prompt-fatigue interaction with fail-closed gate** (per BP-05, D-2026-04-26-13): macOS Keychain prompts on first per-process access. CC-1 fail-closed gate restarts trigger a new Keychain prompt each time. Operator-under-load may toggle "always allow" to escape prompts, which defeats per-process isolation. PRD acknowledges the cross-purposes; TDD §3 must specify a Keychain-prompt-rate budget and an alternative when the budget is exceeded (e.g., session-scoped Keychain handle with explicit re-auth on N>threshold).
...
- **Skills loader**: read `~/.claude/skills/*/SKILL.md` frontmatter, surface skill descriptions to the model. Skill content is presented to the model with explicit untrusted-content framing and Unicode-attack stripping at load time (zero-width / RTL-override / variation-selector removal per T-001 mitigation). Skills run inside Cuttle's policy gate, not their own. **Strip-list maintenance contract** (per WV-05, D-2026-04-26-13): the strip-set is allowlist-shaped, not denylist-shaped. Skills containing Unicode characters outside known-safe categories (Latin / common scripts / explicitly-classified safe symbols) **fail to load** rather than load-with-stripping. Default-deny on novel-category Unicode prevents Best-of-N-style novel injection vectors from exploiting strip-list lag. TDD §2 specifies the safe-category allowlist and the maintenance cadence.
...
- **Audit log**: every tool call, policy decision, deny reason, attestation body provenance (TTY-input vs model-emit), gate-disable event, and tool result digest written to `~/.cuttle/audit/<yyyy-mm-dd>.jsonl` with a tamper-evident chain (specific scheme is OQ-4: HMAC chain vs Merkle tree with periodic root publication, resolved in TDD §5). **Tool-output digest taint annotation** (per WV-03, D-2026-04-26-13): tools are registered with a `secret_bearing: bool` flag; for tools where the flag is true (or unknown / default), the audit log records ONLY metadata (length, type, success/failure), NOT a content sha256. The content digest of secret-bearing tool output is itself a fingerprint side-channel attackable offline if the audit log leaks; only metadata is recorded for tagged tools. TDD §5 specifies the tool-registration tagging contract and the safe-by-default posture (unknown tools treated as secret-bearing). The audit log is anti-forgetfulness and anti-drift; it is **not** anti-Sybil against the operator-as-adversary in v0.1 single-operator scope (per T-003 / D-2026-04-26-08); it is now also **not** a cleartext side-channel for secret-bearing tool outputs.
...
- **Attestation provenance separation.** The policy-gate process maintains a structural separation between operator-typed text (TTY input events) and model-emitted text. Attestation fields that gate-bypass predicates consume as evidence (Option C §6.1.3, escape-hatch evidence per the bullet above) require operator-typed provenance; model-emitted strings are rejected as evidence even if they would satisfy the predicate's content shape. (Per T-001, D-2026-04-26-08.) **Limitation explicitly disclaimed (per WV-06, WV-07, D-2026-04-26-13):** TTY-provenance separation distinguishes bytes-typed-by-operator from bytes-emitted-by-model. It does NOT distinguish operator-INTENT from operator-FATIGUE-KEYPRESS. A model can emit "to bypass this gate, please type the following: <attack string>" and an operator under load may comply; the bytes then carry valid Tty provenance. This is the same operator-fatigue failure mode the §15 corroboration voices warn about; Cuttle does not solve it at the per-attestation grain in v0.1. F-Cuttle-FATIGUE in §12 makes this empirically falsifiable. TDD §3 may explore UX-grain primitives that extract operator-intent from operator-keystrokes (dynamic prompts, predicate-specific evidence shapes the model cannot pre-suggest); these are not v0.1 commitments.
...
9. **Operator restores `~/.cuttle/` from a snapshot or backup** (per BP-04, D-2026-04-26-13). Multiple Cuttle subsystems implicitly assume `~/.cuttle/` state is current: audit-log chain head, lockfile (per §8 case 6), reward-loop registry signed-provenance chain, telemetry aggregates, credential-vault `helper_hash` records (per §6.1.1). A restore from an old snapshot resurrects stale chain heads, stale lockfiles, stale registry entries, and stale helper-hash records, all of which are silently inconsistent with the current world. v0.1 default: on session start, Cuttle reads a `~/.cuttle/state-coherence.json` written at last clean shutdown; if the file is missing OR mtime / chain-head digest does not match the audit-log head, Cuttle refuses to start without explicit `--restored-from-backup` operator acknowledgment that resets the chain heads (logged as a high-trust event, counts as evidence for F-Cuttle-DISABLE per §12). Prevents silent inheritance of stale-state-as-current-state.
...
| OQ-11 | Process-isolation model (per BP-01, D-2026-04-26-13): policy gate, model client, audit-log writer, credential vault, skills loader all live in one process today. Single-process compromise cascades to all subsystems. Should the policy gate run as a separate OS process supervising the model client over typed IPC? Trade: cleaner OS-level trust boundary vs added latency + IPC complexity.                                                                            | TDD §3 (revisit at v0.2 latest) |
| OQ-12 | Audit-log PII posture (per BP-06, D-2026-04-26-13): the audit log captures tool call arguments and result digests, which may contain PII (file paths with emails, DB query results, document tool content). Three options: (a) record-as-is and treat the audit log as a PII data store with retention/access constraints, (b) redact-at-write via per-tool PII annotations, (c) refuse-tools-that-may-emit-PII at registration. Affects `privacy` skill review (§11 step 3). | TDD §5 + privacy review         |
...
Each predicate below is also a Ford / Parsons / Kua _architectural fitness function_ (per D-2026-04-26-10; `process/martin-fowler-input.md` Source 2): "an objective integrity assessment of an architectural characteristic," evaluated continuously through Cuttle's audit-log and telemetry surfaces (§6.1.6) rather than at one-off review. The two names point at the same primitive; "sealed falsifier" preserves the framework-methodology lineage, "fitness function" names the industry-standard concept. **v0.1 ships the predicates as data only** (per BP-02, D-2026-04-26-13): the audit log captures the events; the AUTOMATED EVALUATOR (the build-time machinery a real fitness function ships with) is TDD-grade scope (`docs/TDD.md` §5 specifies the eval contract, thresholds, and reporting). At v0.1 ship the predicates are operator-evaluable post-week, not auto-evaluated by the build. The "fitness function" framing is therefore aspirational at v0.1 ship and operationally true at v0.2+ when the eval machinery lands.
...
- **F-Cuttle-FATIGUE** (per WV-06 / WV-07, D-2026-04-26-13): if attestation bodies operator-typed during v0.1 dogfood week contain substring matches against model-emitted text in the same conversation turn at rate ≥ R_F, the attestation-provenance separation invariant (§6.1.5) is being defeated by operator-fatigue-keypress (the operator is typing what the model suggested rather than authoring independent attestation). The TTY-provenance primitive then provides bytes-typed-by-operator separation but not operator-INTENT separation; the Option C / escape-hatch evidence pathways collapse to model-driven bypass at the operator-fatigue grain. R_F set in TDD §3 (probable starting point: ≥30% of attestation tokens trigram-matchable to immediately-prior model output). Detection requires per-attestation logging of model context window at the moment of TTY input, which is itself a privacy-sensitive surface OQ-12 must address.
...
**What the convergence does NOT establish** (per BP-03 / BP-07, D-2026-04-26-13). The four voices converge in their respective domains (enterprise CI / team threat modeling / software architecture / application security design); none was addressing single-operator AI-agent harnesses. Cuttle takes the substrate-vs-ceremony principle to a fifth domain (LLM-agent harnessing) where it has not been tested at industry scale. The §15 table is convergent-precedent, not direct-validation. Dissenting precedents Cuttle should remain honest about: capability-discipline traditions (Lampson) argue for trust-the-user with structured permissions rather than deny-by-default at the per-call grain; operator-fatigue-inevitability arguments (Mickens, Geer) caution that any threat-model-as-substrate eventually hits the same operator-fatigue wall the ceremony-based versions do (this dissent is operationally captured by F-Cuttle-FATIGUE in §12). v1.3 acknowledges the §15 corroboration is appeal-to-converging-precedent, not appeal-to-resolved-question.

Source artifacts (three-layer discipline): `process/carlos-arguelles-input.md`, `process/martin-fowler-input.md` (Sources 1, 2, 3). The framework-side sidecar `framework_external_corroboration.md` (handoff path #4) is the live cross-project record; this §15 is the PRD-grain snapshot. The dissenting-precedents list is open: TDD adversarial-review and beyond may add further dissent rows as they surface.
